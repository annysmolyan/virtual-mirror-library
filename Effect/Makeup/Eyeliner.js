...
/**
 * Apply eyeliner color effect
 * @type {{apply: EyelinerEffect.apply}}
 */
const EyelinerEffect = (function () {
    ...
    return { // Public Area
        ...
        apply: function (resultCanvasElement, eyesData, effectSettings) {
            ...
            if (!isValidEffectSettings(effectSettings)) {
                throw new Error('Invalid eyeliner effect settings object.');
            }
            ...
            DrawUtility.drawContour(
                maskCanvasContext,
                eyesData.rightPeakWing.concat(eyesData.rightEyeLineBottom).concat(eyesData.rightEyeLineTop.slice(0,5)),
                {fillStyle: effectSettings.value}
            );

            DrawUtility.drawContour(
                maskCanvasContext,
                eyesData.leftEyeLineBottom.concat(eyesData.leftEyeLineTop.slice(2, 5)).concat(eyesData.leftPeakWing),
                {fillStyle: effectSettings.value}
            );
